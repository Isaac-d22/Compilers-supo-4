...
(* (code stack,         value/env stack, state) -> (code stack,  value/env stack, state) *)  
 ...
 | (ASSIGN :: ds,   (V v):: (V (INT l)) :: evs, s)      -> (ds, V(SKIP) :: evs, assign s l v)
 | (DEREF :: ds,   (V(INT l)) :: evs, s)                -> (ds, V(deref s l) :: evs, s)
...
let rec compile = function 
 ...
 | Assign(l, e)   -> (compile e) @ [PUSH (INT l)] @ [ASSIGN]
 | Deref l        -> [PUSH (INT l)] @ [DEREF]
