...
let step = function 
 ...
 | EXAMINE(Assign(l, e),                env, k) -> EXAMINE(e, env, ASN(l, env) :: k)
 | EXAMINE(If(e1, e2, e3),              env, k) -> EXAMINE(e1, env, IF(e2, env, e3) :: k)
 | EXAMINE(While(e1, e2),               env, k) -> EXAMINE(e1, env, WHILE(e1, env, e2) :: k)
 (* EXAMINE --> COMPUTE *) 
 ...
 | COMPUTE(ASN(l, env) :: k, v)   -> COMPUTE(k, let _ = do_assign l v in SKIP)
  (* COMPUTE --> EXAMINE *) 
 | COMPUTE(OPER_FST (e2, env, op) :: k,         v1)  -> EXAMINE(e2, env, OPER (op, v1) :: k)
 | COMPUTE((TAIL (el, env)) :: k,     _)  ->  EXAMINE(Seq el, env, k) 
 | COMPUTE(IF(e2, env, e3) :: k, BOOL(b))    -> if b then EXAMINE(e2, env, k)
                                                else EXAMINE(e3, env, k)
 | COMPUTE(WHILE(e1, env, e2) :: k, BOOL(b)) -> if b then EXAMINE(Seq (e2 :: [While(e1, e2)]), env, k)
                                                else EXAMINE(Skip, env, k)
