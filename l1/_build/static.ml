...
let make_bop loc bop (e1, t1) (e2, t2) = 
    ...
    | GTE, TEint,  TEint  -> (Op(loc, e1, bop, e2), TEbool) 
    | GTE, TEint,  t      -> report_expecting e2 "integer" t
    | GTE, t,      _      -> report_expecting e1 "integer" t
...
let make_assign loc l (e, t) = 
    match t with
    | TEint -> (Assign(loc, l, e), TEunit)
    | _     -> report_expecting e "integer" t

let make_if loc (e1, t1) (e2, t2) (e3, t3) =
    match t1 with
    | TEbool -> if t2 == t3 then (If(loc, e1, e2, e3), t2)
...
let make_while loc (e1, t1) (e2, t2) =
    ...
    | TEbool, _      -> report_expecting e2 "unit" t2
    | _,_            -> report_expecting e1 "boolean" t1
...
let rec  infer env e = 
    ...
    | Assign(loc, l, e)    -> make_assign loc l (infer env e)
    | Deref(loc, l)        -> (Deref(loc, l), TEint)
